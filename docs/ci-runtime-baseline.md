# rusty-detox — CI Runtime Baseline

**Status**: v0.2.0 baseline projection (E011 Phase 8 T095).
...
v0.2.0, rusty-vipe v0.2.0, rusty-pee v0.2.0, and rusty-pwgen v0.2.0
baselines (also zero-leaf single-capability ports).

The full v0.2.0 CI matrix for rusty-detox has these jobs:

| Tier | Job name | OS | Count | Notes |
|------|----------|-----|------:|-------|
| Pre-gates | fmt, clippy, audit, msrv | Linux | 4 | Identical to v0.1.x. |
| Tier 1 | test-default | linux x86_64 + aarch64 + macos x86_64 + macos aarch64 + windows x86_64 | 5 | Full DDR-003 cross-compile matrix (preserves v0.1.x coverage). The Linux x86_64 entry also runs the SC-002 default-install smoke (drives the binary through a messy-name dry-run, `-L` sequence enumeration, `-s utf_8` selection, and the completions subcommand). |
| Tier 2 | test-no-default | Linux x86_64 | 1 | Bare library + SC-001 dep-tree audit. |
| Tier 3 | test-detox-classic, test-detox-minimal | Linux x86_64 | 2 | Preset bundle install + SC-003 size check. |
...
GitHub-hosted matrix). rusty-detox's library API tests cover the
filter pipeline, sequence builder, planner, and renamer (including
EXDEV fallback simulations under `tempfile`).
...
A local `cargo check` matrix run was performed on Windows during T089
validation to confirm Cargo.lock regeneration and per-feature
compilability. Sanitized output:

| Variant | Wall-clock |
|---------|-----------:|
| `cargo check --all-features` (cold) | ~0.6 s |
| `cargo check --no-default-features` | ~0.3 s (deps reused) |
| `cargo check --no-default-features --features cli` | ~0.5 s (warm) |
| `cargo check --no-default-features --features detox-classic` | ~0.5 s (warm) |
| `cargo check --no-default-features --features detox-minimal` | ~0.5 s (warm) |
| `cargo check --no-default-features --features full` | ~0.5 s (warm) |
| `cargo check --no-default-features --features inline-detox` | ~0.5 s (warm) |

These are local-machine checks (toolchain `cargo 1.92.0`,
`rustc 1.92.0`) intended only as a quick sanity check that the
per-feature graph compiles. The CI matrix uses `cargo build/test`
on cold rust-caches per target and will be slower; the empirical CI
capture (3 full runs, median of the slowest matrix entry) is
deferred until the v0.2.0 PR opens its first CI run.
